...
+ If some features from the `x` matrix/data frame contain missing values (NA),
...
```{r expressionset_code, eval = FALSE, message = FALSE, warning = FALSE}
library(Biobase)
...
```{r golub_false, warning = FALSE, message = FALSE}
library(golubEsets)
data(Golub_Merge)
...
The computation results in a signature of 4 features only and a sparse SVM model
performing even better (95.9% accuracy) than the model trained on the dataset
of 500 variables (95.5% accuracy).
...
between the AML and ALL groups [@Golub1999]. Interestingly, the third probe,
